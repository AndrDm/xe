...
extern "C" __declspec(dllexport) void opencv2PyrDown(
		const void *SrcImage, void *DstImage, //Dst is half size as Src
		LVErrorCluster *ErrorCluster)
{
	...
	switch (((ImageInfo *)ImgDst)->imageType){
		...
		case IMAQ_IMAGE_SGL:
			...
			LV_IS_NOT_IMAGE(LVImagePtrDstSGL, ErrorCluster);
			break;
		default:
			ADV_SetLVError(ERR_INVALID_IMAGE_TYPE, __func__, ErrorCluster);
			return;
	...
	Mat dst(LVWidth/2, LVHeight/2, opencv2type);
	...
	switch  (((ImageInfo *)ImgDst)->imageType){
		case IMAQ_IMAGE_U16:
			for (int y = 0; y < LVHeight/2; ++y) {
		    	memcpy (LVImagePtrDstU16 + y * LVLineWidthDst, 
					(unsigned short *)dst.data + y * (LVWidth/2), 
					(LVWidth/2) * sizeof(unsigned short int));
			}		
		break;
		case IMAQ_IMAGE_SGL:
			for (int y = 0; y < LVHeight/2; ++y) {
		    	memcpy (LVImagePtrDstSGL + y * LVLineWidthDst, 
					(float *)dst.data + y * (LVWidth/2), 
					(LVWidth/2) * sizeof(float));
			}		
		break;
		default:
			ADV_SetLVError(ERR_INVALID_IMAGE_TYPE, __func__, ErrorCluster);
...
extern "C" __declspec(dllexport) void opencv2PyrUp(
		const void *SrcImage, void *DstImage, //Dst is double size as Src
		LVErrorCluster *ErrorCluster)
{
	Image *ImgSrc, *ImgDst;
	unsigned short int *LVImagePtrSrcU16, *LVImagePtrDstU16;
